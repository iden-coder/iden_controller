...
WALL_SENSITIVE_POINTS = set(["s3", "s3t", "s4", "s4t", "s9", "s9t"])
TURN_RISK_POINTS = set(["s6t", "s9t"])


def setup_event_log():
    global EVENT_LOG
    path = rospy.get_param('~event_log_path', '/tmp/process_navfn_exp3.log')
    try:
        EVENT_LOG = open(path, 'w')
        EVENT_LOG.write("process_navfn_exp3 start %.3f\n" % time.time())
        EVENT_LOG.flush()
        rospy.loginfo("event log: %s", path)
    except Exception as e:
        EVENT_LOG = None
        rospy.logwarn("event log unavailable: %s", e)


def event_log(message, *args):
    if not EVENT_LOG:
        return
    text = message % args if args else message
    try:
        EVENT_LOG.write("[%8.3f] %s\n" % (time.time(), text))
        EVENT_LOG.flush()
    except Exception:
        pass


def publish_current_point(name):
    try:
        rospy.set_param(CURRENT_POINT_PARAM, name)
    except Exception:
        pass
...
def force_move_x(distance, speed=0.32, direction=1, min_front_clear=0.24):
...
def reverse_escape(distance=0.12, speed=0.06):
    topic = rospy.get_param('~force_cmd_vel_topic', '/cmd_vel_raw')
    pub = rospy.Publisher(topic, Twist, queue_size=10)
    rospy.sleep(0.15)
...
def wall_clearance():
    return min(S.front_min, S.left_min, S.right_min)


def wall_sensitive_accept(name, pose7):
    if name not in WALL_SENSITIVE_POINTS or not scan_is_fresh():
        return False, ""
    dist, yaw_err = goal_error(pose7)
    if dist is None:
        return False, ""

    pos_tol = rospy.get_param('~wall_accept_pos_tolerance', 0.36)
    front_limit = rospy.get_param('~wall_accept_front_clearance', 0.26)
    side_limit = rospy.get_param('~wall_accept_side_clearance', 0.20)
    near_wall = S.front_min < front_limit or S.left_min < side_limit or S.right_min < side_limit
    if near_wall and dist <= pos_tol:
        reason = "pos %.2fm yaw %.1fdeg clear f/l/r %.2f/%.2f/%.2f" % (
            dist, math.degrees(yaw_err), S.front_min, S.left_min, S.right_min)
        return True, reason
    return False, ""


def goto_point_once(name, pose7, hard_timeout=12.0,
                    stuck_window=2.8, stuck_min_dist=0.05,
                    stuck_min_yaw_deg=8.0):
    if name in WALL_SENSITIVE_POINTS:
        hard_timeout = min(hard_timeout, rospy.get_param('~wall_point_timeout', 10.0))
        stuck_window = max(stuck_window, rospy.get_param('~wall_point_stuck_window', 4.5))
    if name in TURN_RISK_POINTS:
        stuck_window = max(stuck_window, rospy.get_param('~turn_point_stuck_window', 4.5))
        stuck_min_yaw_deg = min(
            stuck_min_yaw_deg,
            rospy.get_param('~turn_point_stuck_yaw_deg', 4.0))
    ...
    event_log("[%s] goal x=%.3f y=%.3f", name, pose7[0], pose7[1])
    ...
    while not rospy.is_shutdown():
        ...
        accepted, reason = wall_sensitive_accept(name, pose7)
        if accepted:
            rospy.logwarn("  [%s] near wall, accept and leave: %s", name, reason)
            event_log("[%s] wall accept: %s", name, reason)
            cancel_all()
            stop_robot(0.25)
            return True
        ...
        if state == GoalStatus.SUCCEEDED:
            ...
            if verified:
                ...
                event_log("[%s] reached dist=%.2fm time=%.1fs %s",
                          name, S.odom_dist, elapsed, reason)
                return True
            rospy.logwarn("  [%s] move_base succeeded but target not verified: %s",
                          name, reason)
            event_log("[%s] not verified: %s", name, reason)
            return False
        if state in (GoalStatus.ABORTED, GoalStatus.REJECTED,
            ...
            event_log("[%s] move_base ended: %s", name, goal_state_name(state))
            return False
        if elapsed >= hard_timeout:
            ...
            event_log("[%s] timeout %.1fs dist=%.2fm", name, hard_timeout, S.odom_dist)
            return False
        ...
        if now_t - stuck_time_at >= stuck_window:
            ...
            if moved < stuck_min_dist and not turning:
                ...
                event_log("[%s] stuck moved=%.3fm yaw=%.1fdeg",
                          name, moved, math.degrees(yaw))
                return False
...
def recover_before_retry(name):
    ...
    event_log("[%s] recovery", name)
    cancel_all()
    stop_robot(0.3)
    if scan_is_fresh() and S.front_min < 0.22:
        reverse_escape(distance=0.06, speed=0.05)
    clear_costmaps()
    rospy.sleep(0.6)


def recover_after_failed_point(name):
    rospy.logwarn("  [%s] failed point recovery: stop, tiny escape if needed, continue", name)
    event_log("[%s] failed point recovery before continue", name)
    cancel_all()
    stop_robot(0.3)
    if scan_is_fresh() and S.front_min < 0.24:
        reverse_escape(distance=0.05, speed=0.05)
    clear_costmaps()
    rospy.sleep(0.6)
...
def cruise():
    ...
    for idx, name in enumerate(S.patrol_path):
        ...
        if name == "s15":
            rospy.loginfo("  [s15] clear costmaps before final long leg")
            event_log("[s15] pre-clear costmaps")
            clear_costmaps()
            rospy.sleep(0.2)

        rospy.loginfo("--- [%d/%d] %s ---", idx + 1, total, name)
        event_log("--- [%d/%d] %s ---", idx + 1, total, name)
        publish_current_point(name)
        success = goto_point(name, S.nav_points[name], retries=1)
        if success:
            ok_count += 1
        else:
            fail_list.append(name)
            recover_after_failed_point(name)
            rospy.logerr("patrol point failed at %s after retries, continue route", name)
            event_log("patrol point failed at %s, continue route", name)
            continue

        if success and name == "s13":
            force_move_x(0.60, speed=0.32, direction=1)
    ...
    event_log("patrol finished: success %d/%d, failed %d",
              ok_count, total, len(fail_list))
    if fail_list:
        rospy.logwarn("failed points: %s", ", ".join(fail_list))
        event_log("failed points: %s", ", ".join(fail_list))
    rospy.loginfo("=" * 50)
...
if __name__ == '__main__':
    rospy.init_node('cruise_mode_exp3')
    setup_event_log()
    ...
    send_initial_pose()
    rospy.sleep(1.0)
    rospy.loginfo("start patrol: exp3 navfn mode")
    cruise()
