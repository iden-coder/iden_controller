...
"""Factory-room manager with strict BEV parking-frame handoff and recovery."""
...
from factory_room_vision_parking_v3 import StrictBevParkingDetector


class StrictParkingFactoryRoomManager(FactoryRoomDeliveryManager):
    def __init__(self):
        super(StrictParkingFactoryRoomManager, self).__init__()
        self.square_detector = StrictBevParkingDetector()
        self.parking_square_seed = None

    def square_is_geometrically_valid(self, result):
        if not result or not result.get("found"):
            return False
        width = result.get("rail_width_m")
        near = result.get("near_edge_distance_m")
        if width is None or near is None:
            return False
        min_width = float(rospy.get_param("~parking_frame_min_width_m", 0.30))
        max_width = float(rospy.get_param("~parking_frame_max_width_m", 0.68))
        return (min_width <= float(width) <= max_width and
    ...
    def acquire_square(self, timeout_s=10.0):
        ...
        required = int(rospy.get_param(
            "~parking_acquire_stable_frames", 6))
        stable = 0
        previous = None
        best = None
        start = time.time()
        ...
        while not rospy.is_shutdown() and time.time() - start < timeout_s:
            ...
            if not self.square_is_geometrically_valid(result):
                ...
                rospy.logwarn_throttle(
                    1.0, "WHITE_BOX_BEV_WAIT rails/crossbar not stable")
                rate.sleep()
                continue

            lateral = float(result["lateral_error_m"])
            width = float(result["rail_width_m"])
            consistent = (
                previous is None or
                (abs(lateral - float(previous["lateral_error_m"])) <= 0.05 and
                 abs(width - float(previous["rail_width_m"])) <= 0.10))
            stable = stable + 1 if consistent else 1
            previous = result
            if best is None or float(result["confidence"]) > float(best["confidence"]):
                best = result
            rospy.logwarn_throttle(
                0.5,
                "WHITE_BOX_BEV_STABLE frames=%d/%d conf=%.2f off=%.3fm width=%.3fm near=%.3fm",
                stable, required, float(result["confidence"]), lateral,
                width, float(result["near_edge_distance_m"]))
            if stable >= max(4, required):
                self.parking_square_seed = result
                rospy.logwarn(
                    "WHITE_BOX_BEV_ACQUIRED conf=%.2f off=%.3fm width=%.3fm near=%.3fm",
                    float(result["confidence"]), lateral, width,
                    float(result["near_edge_distance_m"]))
                return result
            rate.sleep()
        return best if best and stable >= max(4, required) else None
    ...
    def park_inside_square(self):
        ...
        stable = 0
        last_result = self.parking_square_seed
        align_start = time.time()
        _, align_origin_yaw = self.snapshot_odom()
        search_direction = 1.0
        if last_result is not None:
            seed_lateral = float(last_result["lateral_error_m"])
            direction = 1.0 if self.image_mirrored else -1.0
            if abs(seed_lateral) > 1.0e-3:
                search_direction = math.copysign(1.0, direction * seed_lateral)
        sweep_limit = math.radians(float(rospy.get_param(
            "~parking_reacquire_sweep_deg", 22.0)))
        search_speed = float(rospy.get_param(
            "~parking_reacquire_speed", 0.10))
        rate = rospy.Rate(10)
        while (not rospy.is_shutdown() and
            ...
            if not self.square_is_geometrically_valid(result):
                ...
                if (align_origin_yaw is None or current_yaw is None or
                        clearance < self.spin_clearance_m):
                    self.publish_zero()
                else:
                    swept = norm_angle(current_yaw - align_origin_yaw)
                    if swept >= sweep_limit:
                        search_direction = -1.0
                    elif swept <= -sweep_limit:
                        search_direction = 1.0
                    command = Twist()
                    command.angular.z = search_direction * search_speed
                    self.cmd_pub.publish(command)
                rospy.logwarn_throttle(
                    0.8,
                    "WHITE_BOX_REACQUIRE swept=%.1fdeg dir=%+.0f clearance=%.3fm",
                    math.degrees(norm_angle(current_yaw - align_origin_yaw))
                    if current_yaw is not None and align_origin_yaw is not None else 0.0,
                    search_direction, clearance)
                rate.sleep()
                continue
...
if __name__ == "__main__":
    node = StrictParkingFactoryRoomManager()
    rospy.spin()
