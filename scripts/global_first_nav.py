...
class GlobalFirstPlanner:
    ...
    def plan(self, start_world, goal_world):
        hard = float(self.params.get("hard_clearance_m", 0.10))
        ...
        if allow_fallback and emergency < hard:
            mid = (hard + emergency) * 0.5
            attempts.extend([mid, emergency])
    ...
    def _plan_with_clearance(self, start_world, goal_world, clearance_m):
        ...
        max_time = float(self.params.get("max_planning_time_s", 3.0))
        ...
        cells = self._astar(start_cell, goal_cell, clearance_m, max_time)
        if not cells:
            return {"ok": False, "error": "A* failed at clearance %.3fm" % clearance_m}
        ...
        min_clearance = min(self.grid.clearance_m(x, y) for x, y in cells)
        length = self._path_length(path_world)
    ...
    def _astar(self, start, goal, clearance_m, max_time):
        w = self.grid.width
        h = self.grid.height
        n = w * h
        ...
        preferred = float(self.params.get("preferred_clearance_m", 0.23))
        prox_weight = float(self.params.get("proximity_weight", 2.2))
        unknown_cost = float(self.params.get("unknown_cost", 4.0))
        heuristic_weight = float(self.params.get("heuristic_weight", 1.08))
        ...
        while heap:
            ...
            for dx, dy, step in dirs:
                ...
                if preferred > clearance_m and clear < preferred:
                    t = (preferred - clear) / max(preferred - clearance_m, 1e-6)
...
def offline_main(argv):
    ...
    parser.add_argument("--hard-clearance", type=float, default=0.10)
    parser.add_argument("--preferred-clearance", type=float, default=0.23)
    parser.add_argument("--emergency-clearance", type=float, default=0.07)
    ...
    planner = GlobalFirstPlanner(grid, params)
...
class RosGlobalFirstNavigator:
    def __init__(self):
        ...
        rospy.init_node("global_first_nav")
        ...
        self.params = {
            "hard_clearance_m": rospy.get_param("~hard_clearance_m", 0.10),
            "preferred_clearance_m": rospy.get_param("~preferred_clearance_m", 0.23),
            "emergency_min_clearance_m": rospy.get_param(
                "~emergency_min_clearance_m", 0.07),
            "smooth_clearance_m": rospy.get_param("~smooth_clearance_m", 0.10),
            "allow_clearance_fallback": rospy.get_param(
                "~allow_clearance_fallback", True),
            "goal_snap_radius_m": rospy.get_param("~goal_snap_radius_m", 0.45),
            "max_planning_time_s": rospy.get_param("~max_planning_time_s", 4.0),
            "proximity_weight": rospy.get_param("~proximity_weight", 2.4),
            "unknown_cost": rospy.get_param("~unknown_cost", 6.0),
            "heuristic_weight": rospy.get_param("~heuristic_weight", 1.08),
        ...
        self.lookahead_dist = rospy.get_param("~lookahead_dist_m", 0.32)
        self.rotate_in_place_angle = math.radians(
            rospy.get_param("~rotate_in_place_angle_deg", 42.0))
        self.max_linear = rospy.get_param("~max_linear_vel", 0.28)
        self.min_tracking_speed = rospy.get_param("~min_tracking_speed", 0.045)
        self.max_angular = rospy.get_param("~max_angular_vel", 0.78)
        ...
        self.k_heading = rospy.get_param("~k_heading", 1.35)
        self.k_lateral = rospy.get_param("~k_lateral", 0.65)
        self.linear_accel = rospy.get_param("~linear_accel", 0.22)
        self.angular_accel = rospy.get_param("~angular_accel", 1.00)

        self.front_angle_deg = rospy.get_param("~front_angle_deg", 34.0)
        self.side_angle_min_deg = rospy.get_param("~side_angle_min_deg", 35.0)
        self.side_angle_max_deg = rospy.get_param("~side_angle_max_deg", 78.0)
        self.front_stop_m = rospy.get_param("~front_stop_m", 0.19)
        self.front_slow_m = rospy.get_param("~front_slow_m", 0.54)
        self.side_stop_m = rospy.get_param("~side_stop_m", 0.105)
        self.side_slow_m = rospy.get_param("~side_slow_m", 0.30)
        self.scan_timeout_s = rospy.get_param("~scan_timeout_s", 0.70)
        self.pose_timeout_s = rospy.get_param("~pose_timeout_s", 5.0)
        self.front_turn_speed = rospy.get_param("~front_turn_speed", 0.24)
        self.front_replan_after_s = rospy.get_param("~front_replan_after_s", 4.0)

        self.replan_min_interval_s = rospy.get_param("~replan_min_interval_s", 1.6)
        self.blocked_replan_s = rospy.get_param("~blocked_replan_s", 1.2)
        self.stuck_timeout_s = rospy.get_param("~stuck_timeout_s", 7.0)
        self.progress_dist_m = rospy.get_param("~progress_dist_m", 0.07)
        ...
        self.log_status("global_first_nav started; waiting for pose/scan")
    ...
    def load_static_map(self):
        ...
        try:
            ...
            msg = srv().map
            rospy.logwarn("GlobalFirstNav: loaded static map from %s", map_service)
        except Exception as exc:
            rospy.logwarn("GlobalFirstNav: static map service failed: %s", str(exc))

        if msg is None:
            try:
                msg = rospy.wait_for_message(map_topic, OccupancyGrid, timeout=5.0)
                rospy.logwarn("GlobalFirstNav: loaded static map from %s", map_topic)
            except Exception as exc:
                rospy.logwarn("GlobalFirstNav: map topic failed: %s", str(exc))
        ...
        if msg is not None:
            ...
            rospy.logwarn("GlobalFirstNav: loaded map yaml fallback %s", map_yaml)
        else:
            raise RuntimeError("no static map available")

        t0 = time.time()
        self.grid.compute_distance_field()
        self.planner = GlobalFirstPlanner(self.grid, self.params)
        rospy.logwarn("GlobalFirstNav: distance field ready in %.2fs",
                      time.time() - t0)
    ...
    def plan_from_current_pose(self, reason, force=False):
        ...
        self.rospy.logwarn("GlobalFirstNav: planning %s from (%.2f, %.2f) to (%.2f, %.2f)",
        ...
        self.publish_path()
        self.log_status(
            "plan ok: points=%d length=%.2fm min_clear=%.2fm clearance_used=%.2fm" %
            (len(self.path_world), result["path_length_m"],
             result["min_clearance_m"], result["clearance_used_m"]))
        return True
    ...
    def start_pose_ok(self):
        ...
        self.rospy.logwarn_throttle(
            1.5,
            "GlobalFirstNav: waiting for initial pose near (%.2f, %.2f, %.1fdeg); current=(%.2f, %.2f, %.1fdeg), dist=%.2fm",
        ...
        if self.require_goal_yaw:
            ...
            if not yaw_ok:
                if min(self.left, self.right) < self.side_stop_m:
                    ...
                    wz = clamp(1.0 * yaw_err, -0.32, 0.32)
    ...
    def compute_cmd(self, target):
        ...
        heading = math.atan2(local_y, max(local_x, 1e-4))
        if local_x < -0.04:
    ...
    def apply_scan_guard(self, cmd):
        ...
        if self.front < self.front_stop_m:
            ...
            if (blocked_for >= self.front_replan_after_s and
                ...
                self.log_status(
                    "front blocked %.2fm for %.1fs: global replan requested" %
        ...
        if self.front < self.front_slow_m:
            t = (self.front - self.front_stop_m) / max(
                self.front_slow_m - self.front_stop_m, 1e-3)
        ...
        if side_min < self.side_stop_m:
            ...
            t = (side_min - self.side_stop_m) / max(
                self.side_slow_m - self.side_stop_m, 1e-3)
    ...
    def publish_zero(self, reason):
        ...
        self.rospy.logwarn_throttle(2.0, "GlobalFirstNav zero: %s", reason)
    ...
    def log_status(self, text):
        self.rospy.logwarn("GlobalFirstNav: %s", text)
        msg = self.String()
        msg.data = text
