...
class Xunfei2026RoomDeliveryManager(object):
    ...
    def ocr_callback(self, msg):
        ...
        if ignored_label:
            ...
            released = (
                ...
                (blank_long_enough and
                 (moved >= self.non_target_release_distance or
                  yaw_delta >= self.non_target_release_yaw)))
            with self.lock:
            ...
            if not released:
                rospy.logwarn_throttle(
                    ...
                    "moved=%.2f yaw=%.1fdeg scan_continues=true",
            ...
            rospy.logwarn(
                "OCR_NON_TARGET_VIEW_RELEASED label=%s moved=%.2f "
                "yaw=%.1fdeg votes_reset=true", ignored_label, moved,
                math.degrees(yaw_delta))
            return
        ...
        if (isinstance(bbox, (list, tuple)) and len(bbox) >= 4 and
                width > 1.0):
            margin = self.target_edge_margin_ratio * width
            bbox_visible = (float(bbox[0]) >= margin and
                            float(bbox[2]) <= width - margin)
        current_target = (
            stable and stable_label == target and frame_label == target and
            votes >= self.required_ocr_votes and bbox_visible)
        with self.lock:
        ...
        if (current_target and confirm_count >= 1 and
                now >= self.candidate_cooldown_until):
            with self.lock:
                self.candidate_last_seen = now
                # Lock only OCR belonging to the requested workshop.  A nearby
                # different sign must never invalidate parking after handoff.
    ...
    def side_footprint_clearance(self, vy, front_wall_distance=None):
        ...
        for _, _, x_base, y_base in samples:
            ...
            if front_wall_distance is not None:
                x_laser = x_base - self.laser_offset_x
    ...
    def follow_wall_segment(self, index):
        ...
        while not rospy.is_shutdown() and time.monotonic() < deadline:
            ...
            if side_body is not None:
                body_ratio = clamp(
                    ...
                    max(0.02, self.side_body_slow_gap -
                        self.side_body_hard_gap), 0.0, 1.0)

            slow_ratio = clamp(
                (side_clearance - self.cone_side_stop) /
                max(0.02, self.cone_side_slow - self.cone_side_stop),
                0.0, 1.0)
            lateral_speed = self.wall_route_min_speed + (
                self.wall_route_speed - self.wall_route_min_speed) * slow_ratio
            lateral_speed *= body_ratio
            if remaining < 0.28:
            ...
            # Continuous cone avoidance: bend away from the obstacle while
            # retaining forward progress.  No fixed retreat distance or
            # stop-and-restart manoeuvre is involved.
            avoid_ratio = clamp(
                (self.cone_side_slow - side_clearance) /
                max(0.02, self.cone_side_slow - self.cone_side_hard),
                0.0, 1.0)
            avoid_vx = -self.cone_avoid_max_vx * avoid_ratio
            if avoid_ratio > 0.0:
                command.linear.x = min(command.linear.x, avoid_vx)
            ...
            # At the true footprint boundary, stop advancing toward the cone
            # but continue the clearance-increasing component.  As soon as the
            # scan opens, lateral progress resumes automatically.
            command.linear.y = (0.0 if side_clearance <= self.cone_side_hard
                                else -lateral_speed)
            if side_body is not None and side_body[0] <= self.side_body_hard_gap:
                command.linear.y = 0.0
                command.linear.x = self.side_escape_velocity(
                    side_body, "WALL_ROUTE")
                rospy.logwarn_throttle(
                    0.35, "WALL_ROUTE_SIDE_BODY_BLOCKED gap=%.3f "
                    "point=(%.3f,%.3f) escape_vx=%.3f",
                    side_body[0], side_body[1], side_body[2],
                    command.linear.x)
            ...
            rospy.logwarn_throttle(
                0.5,
                "WALL_ROUTE segment=%s progress=%.2f/%.2fm wall=%.2f/%.2fm "
                "side=%.2fm avoid=%.2f cmd=(%.2f,%.2f,%.2f)",
                segment_name, progress, length, wall_distance, wall_target,
    ...
    def run_wall_route(self):
        start_name, start_x, start_y = self.wall_route_points[0]
        start_yaw = self.wall_route_yaws[0]
        ...
        if result != "SUCCEEDED":
            ...
            replacement = self.nearest_route_start(start_x, start_y, start_yaw)
        ...
        if result == "TARGET":
            return result
        if result != "SUCCEEDED":
            # Being near the intended corridor is enough; direct wall fitting
            # can recover from a loose move_base endpoint.
            pose = self.current_pose()
            if pose is None or math.hypot(
                    pose[0] - start_x, pose[1] - start_y) > 0.75:
                return result
            self.publish_state("WALL_ROUTE_START_ACCEPTED_LOOSE",
                               result=result, pose_x=pose[0], pose_y=pose[1])

        for index in range(len(self.wall_route_points) - 1):
            ...
            if result not in ("SUCCEEDED", "SUCCEEDED_FALLBACK"):
                ...
                if replacement is not None:
                    recovery = self.send_goal(
                        replacement[0], replacement[1], replacement[2],
                        "{}_segment_recovery".format(start_name),
    ...
    def parking_raw_sector_clearance(self, center_angle, half_angle):
        ...
        for _, _, x_base, y_base in samples:
            x_laser = x_base - self.laser_offset_x
            distance = math.hypot(x_laser, y_base)
            angle = math.atan2(y_base, x_laser)
            if abs(norm_angle(angle - center_angle)) <= half_angle:
    ...
    def parking_commands(self, allow_forward):
        ...
        if target is not None:
            ...
            if target_live or target_age <= self.parking_target_memory_s:
                ...
                if (abs(pixel_error) > self.parking_center_tolerance and
                        abs(vy) < self.parking_min_vy):
                    vy = math.copysign(self.parking_min_vy, vy)
                if abs(pixel_error) <= self.parking_center_tolerance:
                    vy = 0.0
        if allow_forward:
        ...
        target_centered_for_motion = (
            target is None or target_age > self.parking_target_memory_s or
    ...
    def park(self):
        ...
        while not rospy.is_shutdown() and time.monotonic() < deadline:
            ...
            rospy.logwarn_throttle(
                ...
                "pixel=%.1f live=%s cmd=(%.3f,%.3f,%.3f) stable=%d/%d",
                distance, math.degrees(heading), pixel_error,
                str(target_live), vx, vy, wz, align_stable,
        ...
        while not rospy.is_shutdown() and time.monotonic() < deadline:
            ...
            (vx, vy, wz, distance, heading, points, pixel_error,
             target_live, target_age) = values
            self.publish_direct_command(vx, vy, wz)
            centered = self.parking_center_aligned
            if target_live:
                centered = abs(pixel_error) <= self.parking_recenter_threshold
            ...
            rospy.logwarn_throttle(
                ...
                pixel_error, str(target_live), vx, vy, wz, stable,
                self.parking_stable_frames)
    ...
    def mission_thread(self):
        ...
        try:
            ...
            if found:
                self.publish_state("TARGET_FOUND", target=self.target_warehouse)
                self.handoff_directly_to_parking()
                parking_success = self.park()
