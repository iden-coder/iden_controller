...
class HolonomicParkingSafetyMonitor:
    def __init__(self):
        ...
        self.parking_mode_timeout = rospy.get_param(
            "~parking_mode_timeout_s", 0.8)
        self.parking_front_stop = rospy.get_param(
            "~parking_front_stop_m", 0.095)
        self.parking_front_critical = rospy.get_param(
            "~parking_front_critical_m", 0.075)
        self.parking_front_slow = rospy.get_param(
            "~parking_front_slow_m", 0.20)
        self.parking_max_lateral = rospy.get_param(
            "~parking_max_lateral_speed", 0.08)
        self.parking_mode = False
        self.parking_mode_time = rospy.Time(0)
        ...
        self.sub_parking_mode = rospy.Subscriber(
            self.parking_mode_topic, Bool, self.cb_parking_mode, queue_size=1)
    ...
    def cb_parking_mode(self, msg):
        self.parking_mode = bool(msg.data)
        self.parking_mode_time = rospy.Time.now()
        if self.parking_mode:
            self.escape_until = rospy.Time(0)
        rospy.logwarn_throttle(
            1.0, "SafetyMonitor parking_close_mode=%s",
            str(self.parking_mode))

    def parking_mode_active(self):
        if not self.parking_mode:
            return False
        fresh = ((rospy.Time.now() - self.parking_mode_time).to_sec() <=
                 self.parking_mode_timeout)
        if not fresh:
            self.parking_mode = False
            rospy.logerr("parking safety heartbeat lost; normal clearance restored")
        return fresh
    ...
    def compute(self, vx, wz):
        ...
        front_stop = self.parking_front_stop if parking_close else self.front_stop
        front_critical = (self.parking_front_critical if parking_close
                          else self.front_critical)
        front_slow = self.parking_front_slow if parking_close else self.front_slow

        if self.min_front < front_critical:
            escape = None if parking_close else self.maybe_escape("FRONT_CRITICAL", now)
        ...
        if self.min_front < front_stop:
            escape = None if parking_close else self.maybe_escape("STOP_FRONT", now)
            if escape is not None:
                return escape
            safe_wz = self.turn_away_from_wall(wz)
        ...
        if vx > 0.0 and self.min_front < front_slow:
            t = (self.min_front - front_stop) / max(front_slow - front_stop, 1e-3)
    ...
    def cb_cmd(self, msg):
        ...
        parking_close = self.parking_mode_active()
        vy = clamp(msg.linear.y, -self.parking_max_lateral,
                   self.parking_max_lateral) if parking_close else 0.0
        if (action in ("NO_SCAN", "SCAN_TIMEOUT", "FRONT_CRITICAL",
                       "SIDE_CRITICAL", "STOP_FRONT", "STOP_SIDE",
                       "STOP_REAR") or "ESCAPE" in action):
            vy = 0.0
        if vy > 0.0 and self.min_left < self.side_stop:
            vy = 0.0
        if vy < 0.0 and self.min_right < self.side_stop:
            vy = 0.0
        side_min = min(self.min_left, self.min_right)
        if side_min < self.side_slow:
            ratio = clamp(
                (side_min - self.side_stop) /
                max(self.side_slow - self.side_stop, 1.0e-3), 0.35, 1.0)
            vy *= ratio
        out = Twist()
        out.linear.x = vx
        out.linear.y = vy
        ...
        if action not in ("CLEAR", "DISABLED"):
            rospy.logwarn_throttle(
                0.7,
                "HolonomicSafetyMonitor: %s | f=%.2f l=%.2f r=%.2f rear=%.2f | in=(%.3f,%.3f,%.3f) out=(%.3f,%.3f,%.3f)",
...
if __name__ == "__main__":
    try:
        HolonomicParkingSafetyMonitor().run()
