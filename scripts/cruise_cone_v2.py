...
WALL_GOAL_XY_TOL = rospy.get_param("~wall_goal_xy_tolerance", 0.06)
STRICT_FINAL_YAW = rospy.get_param("~strict_final_yaw", True)
FINAL_YAW_TOL_DEG = rospy.get_param("~final_yaw_tolerance_deg", 2.0)
FINAL_YAW_TIMEOUT = rospy.get_param("~final_yaw_timeout", 14.0)
...
BACKUP_DIST = rospy.get_param("~backup_dist", 0.04)
BACKUP_SPEED = rospy.get_param("~backup_speed", -0.04)
# 倒退安全阈值：后方/侧后方太近时，立即禁止/停止倒退。
REAR_OBSTACLE_STOP_DIST = rospy.get_param("~rear_obstacle_stop_dist", 0.18)
CONE_BACKUP_DIST = rospy.get_param("~cone_backup_dist", 0.025)
CONE_BACKUP_SPEED = rospy.get_param("~cone_backup_speed", -0.035)
CONE_BACKUP_MIN_REAR_CLEARANCE = rospy.get_param("~cone_backup_min_rear_clearance", 0.35)
...
NO_AVOID_NEAR_GOAL = rospy.get_param("~no_avoid_near_goal_dist", 0.34)  # 距目标34cm内不绕障
...
REACTIVE_RECOVERY_ATTEMPTS = int(rospy.get_param("~reactive_recovery_attempts", 6))
REACTIVE_RECOVERY_DURATION = rospy.get_param("~reactive_recovery_duration", 10.0)
CONE_STUCK_WINDOW = rospy.get_param("~cone_stuck_window", 4.0)
CONE_STUCK_MIN_DIST = rospy.get_param("~cone_stuck_min_dist", 0.03)
CONE_DIRECT_FINISH_DIST = rospy.get_param("~cone_direct_finish_dist", 0.22)
CONE_CLOSE_FINISH_DIST = rospy.get_param("~cone_close_finish_dist", 0.48)
CONE_CLOSE_FINISH_ATTEMPT = int(rospy.get_param("~cone_close_finish_attempt", 2))
CONE_CLOSE_RECOVERY_DIST = rospy.get_param("~cone_close_recovery_dist", 0.58)
CONE_REACTIVE_DRIFT_LIMIT = rospy.get_param("~cone_reactive_drift_limit", 0.35)
CONE_REACTIVE_DRIFT_GRACE = rospy.get_param("~cone_reactive_drift_grace", 2.5)
...
def nudge_to_target(dx, dy, target_yaw, max_dist=0.22, speed=0.05):
...
def reactive_avoidance(goal_x, goal_y, max_duration=15.0):
    ...
    ALLOW_REVERSE = rospy.get_param("~avoid_allow_reverse", False)
    REVERSE_CLEARANCE = rospy.get_param("~avoid_reverse_clearance", 0.38)
    ...
    while not rospy.is_shutdown():
        ...
        if best_dist is None or dist < best_dist - 0.03:
            best_dist = dist
        elif (elapsed > CONE_REACTIVE_DRIFT_GRACE and
              best_dist is not None and
              dist > best_dist + CONE_REACTIVE_DRIFT_LIMIT):
            rospy.logwarn(f"  [反应式] 距目标从{best_dist:.2f}m变到{dist:.2f}m，提前退出避免绕远")
            pub_cmd.publish(Twist())
            stop_robot(0.2)
            return False
        ...
        if dist < NORMAL_GOAL_XY_TOL or dist <= CONE_DIRECT_FINISH_DIST:
        ...
        if f30 < DANG_F:
            ...
            if dist < 0.30:
                ...
                rear = rear_clearance()
                if ALLOW_REVERSE and rear > REVERSE_CLEARANCE:
                    tvx = -0.035 * speed_scale
                else:
                    tvx = 0.0
                tvz = 0.75 * tdir * turn_scale
                lock_cnt += 1
...
def goto_point(name, pose7, hard_timeout=15.0):
    ...
    while True:  # 不允许跳点，无限重试直到到达
        ...
        while not rospy.is_shutdown():
            ...
            if cone_zone and pos_dist <= CONE_DIRECT_FINISH_DIST:
                rospy.logwarn(f"  [{name}] 锥桶近目标 dist={pos_dist:.2f}m，转入最终精修")
                return finish_goal(name, pose7, align_yaw=True, hold_sec=ARRIVAL_HOLD_SEC)
        ...
        if near_dist <= NO_AVOID_NEAR_GOAL:
            ...
            return finish_goal(name, pose7, align_yaw=True, hold_sec=ARRIVAL_HOLD_SEC)
        if (cone_zone and attempt >= CONE_CLOSE_FINISH_ATTEMPT and
                near_dist <= CONE_CLOSE_FINISH_DIST):
            rospy.logwarn(f"  [{name}] 锥桶近目标多次受阻 dist={near_dist:.2f}m，转入最终精修")
            return finish_goal(name, pose7, align_yaw=True, hold_sec=ARRIVAL_HOLD_SEC)
        ...
        # 卡死恢复：锥桶区尽量不倒车，避免后退时碰到锥桶；普通区才使用小倒车。
        if BACKUP_ON_STUCK:
            if cone_zone:
                rear = rear_clearance()
                if near_dist <= CONE_CLOSE_RECOVERY_DIST:
                    rospy.logwarn(f"  [{name}] 锥桶近目标 dist={near_dist:.2f}m，跳过倒车，仅小角度重规划")
                    stop_robot(0.4)
                elif rear >= CONE_BACKUP_MIN_REAR_CLEARANCE:
                    rospy.logwarn(f"  [{name}] 锥桶区后方{rear:.2f}m，可执行极短微倒{CONE_BACKUP_DIST:.3f}m")
                    backup_distance(CONE_BACKUP_DIST, CONE_BACKUP_SPEED,
                                    timeout=max(1.2, CONE_BACKUP_DIST / max(abs(CONE_BACKUP_SPEED), 0.02) + 0.8))
                else:
                    rospy.logwarn(f"  [{name}] 锥桶区后方仅{rear:.2f}m，跳过倒车，改用小角度转向/重规划")
                    stop_robot(0.4)
            else:
                backup_distance(BACKUP_DIST, BACKUP_SPEED,
                                timeout=max(1.8, BACKUP_DIST / max(abs(BACKUP_SPEED), 0.02) + 0.8))
            rospy.sleep(0.3)

        if update_robot_pose():
            face_yaw = math.atan2(goal_pose[1] - S.map_y, goal_pose[0] - S.map_x)
            if cone_zone and near_dist <= CONE_CLOSE_RECOVERY_DIST:
                offset_angles = [0, -8, 8, 0]
            else:
                offset_angles = [-18, 18, -30, 30, -45, 45] if cone_zone else [-30, -60, 30, 60, -90, 90]
            offset = math.radians(offset_angles[attempt % len(offset_angles)])
            target_yaw = face_yaw + offset
            recovery_label = "锥桶小角度转向" if cone_zone else "倒退后旋转"
            rospy.logwarn(f"  [{name}] {recovery_label}{offset_angles[attempt % len(offset_angles)]}°(偏移), 重新规划")
...
if __name__ == '__main__':
    rospy.init_node('cruise_cone_v2')
