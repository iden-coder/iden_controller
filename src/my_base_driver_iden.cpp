#include <iden_controller/base_driver_iden.h>
#include <Eigen/Eigen>
#include <tf/tf.h>
...
namespace ucarController
{
  ...
  baseBringup::baseBringup() : x_(0), y_(0), th_(0)
  {
    ...
    Mileage_file_name_ = ros::package::getPath("iden_controller") + std::string("/log_info/") + Mileage_file_name_;
    ...
    // PID 闭环参数（X/Y/θ 三轴）
    pravite_nh.param("enable_pid", enable_pid_, true);
    pravite_nh.param("kp_vx", kp_vx_, 0.5);
    pravite_nh.param("ki_vx", ki_vx_, 0.2);
    pravite_nh.param("kd_vx", kd_vx_, 0.0);
    pravite_nh.param("kp_vy", kp_vy_, 0.5);
    ...
    pravite_nh.param("integral_limit_vy", integral_limit_vy_, 0.5);
    pravite_nh.param("integral_limit_vth", integral_limit_vth_, 0.3);

    // 加速度斜坡 (m/s^2, rad/s^2)
    pravite_nh.param("max_accel_x", max_accel_x_, 1.0);
    pravite_nh.param("max_accel_y", max_accel_y_, 1.0);
    ...
    actual_vx_ = actual_vy_ = actual_vth_ = 0.0;

    // 初始化速度平滑状态变量
    prev_ramp_linear_x_ = prev_ramp_linear_y_ = prev_ramp_angular_z_ = 0.0;
    filtered_cmd_linear_x_ = filtered_cmd_linear_y_ = filtered_cmd_angular_z_ = 0.0;
    ...
    while (ros::ok())
    {
      try
      {
        ...
        case MOTOR_MODE_CMD:
        {
          ...
          // 低通滤波平滑 cmd_vel 高频噪声
          double loop_dt = 1.0 / rate_;
          filtered_cmd_linear_x_ = cmd_vel_filter_alpha_ * linear_x
              + (1.0 - cmd_vel_filter_alpha_) * filtered_cmd_linear_x_;
          filtered_cmd_linear_y_ = cmd_vel_filter_alpha_ * linear_y
              + (1.0 - cmd_vel_filter_alpha_) * filtered_cmd_linear_y_;
          filtered_cmd_angular_z_ = cmd_vel_filter_alpha_ * angular_z
              + (1.0 - cmd_vel_filter_alpha_) * filtered_cmd_angular_z_;

          // 加速度斜坡限幅，避免速度阶跃
          double max_dx = max_accel_x_ * loop_dt;
          double max_dy = max_accel_y_ * loop_dt;
          double max_dth = max_accel_th_ * loop_dt;
          double dx = filtered_cmd_linear_x_ - prev_ramp_linear_x_;
          double dy = filtered_cmd_linear_y_ - prev_ramp_linear_y_;
          double dth = filtered_cmd_angular_z_ - prev_ramp_angular_z_;
          dx = std::max(-max_dx, std::min(max_dx, dx));
          dy = std::max(-max_dy, std::min(max_dy, dy));
          dth = std::max(-max_dth, std::min(max_dth, dth));
          prev_ramp_linear_x_ += dx;
          prev_ramp_linear_y_ += dy;
          prev_ramp_angular_z_ += dth;

          linear_x = prev_ramp_linear_x_;
          linear_y = prev_ramp_linear_y_;
          angular_z = prev_ramp_angular_z_;
          break;
        }
        ...
        {
          lock.lock();
          double actual_vx = actual_vx_;
          double actual_vy = actual_vy_;
          double actual_vth = actual_vth_;
          lock.unlock();

          double error_vx = linear_x - actual_vx;
          double error_vy = linear_y - actual_vy;
          double error_vth = angular_z - actual_vth;

          ros::Time now = ros::Time::now();
          double dt = (now - last_pid_time_).toSec();
        ...
        case iden_controller::SetLEDMode::Request::MODE_NORMAL:
        {
          lock.lock();
        ...
        case iden_controller::SetLEDMode::Request::MODE_BLINK:
        {
        ...
        case iden_controller::SetLEDMode::Request::MODE_BREATH:
        {
  ...
  bool baseBringup::getBatteryStateCB(iden_controller::GetBatteryInfo::Request &req,
                                      iden_controller::GetBatteryInfo::Response &res)
  {
  ...
  bool baseBringup::setLEDCallBack(iden_controller::SetLEDMode::Request &req,
                                   iden_controller::SetLEDMode::Response &res)
  {
    if (!read_first_)
    {
      res.success = false;
  ...
  bool baseBringup::getMaxVelCB(iden_controller::GetMaxVel::Request &req, iden_controller::GetMaxVel::Response &res)
  {
    boost::unique_lock<boost::recursive_mutex> lock(Control_mutex_);
  ...
  bool baseBringup::setMaxVelCB(iden_controller::SetMaxVel::Request &req, iden_controller::SetMaxVel::Response &res)
  {
    boost::unique_lock<boost::recursive_mutex> lock(Control_mutex_);
  ...
  void baseBringup::processOdometry()
  {
    ...
    // 存储实际速度供 PID 闭环使用（在锁内完成，避免与 writeLoop 的读取竞争）
    actual_vx_ = Vx;
    actual_vy_ = Vy;
    actual_vth_ = Vth;
    lock.unlock();
